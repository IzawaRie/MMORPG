...
namespace Assets.Scripts.Services
{
    class BattleService:Singleton<BattleService>,IDisposable
    {
        ...
        public void SendSkillCast(int skillId,int casterId,int tergetId,NVector3 position)
        {
            if(position == null) position = new NVector3();
            Debug.LogFormat("SendSkillCast:skill:{0} caster:{1} target:{2} pos:{3}",skillId,casterId, tergetId, position.ToString());
            ...
            message.Request.skillCast.castInfo.targetId = tergetId;
        ...
        private void OnBuff(object sender, BuffResponse message)
        {
            ...
            {
                Debug.LogFormat("    Buff:{0}:{1}[{2}]", buff.casterId, buff.buffType, buff.Action);
